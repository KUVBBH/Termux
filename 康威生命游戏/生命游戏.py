...
#参考:https://baijiahao.baidu.com/s?id=1653854876062433235&wfr=spider&for=pc&searchword=生命游戏结构
####################################
a=np.zeros((100,100))
key=list(name.keys())
print('\n'*50)
...
video = cv2.VideoWriter(sys.path[0]+'/out.mp4', fourcc, cap_fps, (2000,2000),True)
mv=1
...
while True :
    ...
    img=cv2.imread(path)
    cv2.rectangle(img,(0,0),(2000,2000),(0,255,0),10)
    video.write(img)
    for i in W :
        a[i[0],i[1]] = 0
    for i in B :
